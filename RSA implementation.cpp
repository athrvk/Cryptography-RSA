...
void encryption() {
	...
	for(int i = 0; i < message.size(); i++) cout << cipher((int)message.at(i), intToBinary(e)) << " ";
...
int choice() {
	...
	cout << "Generate Keys (0), Encryption (1), Decryption (2), or EXIT (anything else)? ";
	string decision;
	cin >> decision;
	if(decision == "0")
		return 0;
...
void generateKeys() {
	...
	e = (trial * r + 1) / d;
	cout << "Choose a non-negative integer k to set public key = " << e << " + " << r << "k: ";
